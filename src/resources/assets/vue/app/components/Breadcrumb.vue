  ...
  <nav aria-label="breadcrumb">
    <ol class="breadcrumb">
      <li class="breadcrumb-item" v-if="isRouterBackExists">
        <a @click="goBack" href="#">&larr; {{$t('backButtonLabel')}}</a>
      </li>
      <li
        class="breadcrumb-item"
        v-for="(brd, key) in breadcrumbList"
        :class="{'active': key+1 == breadcrumbList.length}"
      >
        <router-link
          v-if="brd.path && key+1 != breadcrumbList.length"
          :to="brd.path"
        >{{$t(brd.name)}}</router-link>
        <span v-else>{{$t(brd.name)}}</span>
      </li>
    </ol>
  </nav>
</template>
