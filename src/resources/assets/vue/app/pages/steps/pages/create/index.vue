  ...
  <div>
    <form @submit="createNewStep">
      ...
      >
        ...
        <label :class="{'text-danger': errorList[model].length}" :for="model+'Element'" v-text="$t(model+'Label')"></label>
        <input
          class="custom-input"
          :class="{'invalid': errorList[model].length}"
          :id="model+'Element'"
      ...
      <button type="submit" class="btn btn-success">{{$t('stepCreateLabel')}}</button>
    </form>
  </div>
</template>
<script>
...
export default {
  ...
  mounted() {
    this.form.options = this.getOptionListAsObject();
  },
  methods: {
    getOptionListAsObject() {
      let passed = {};
      Object.keys(this.defaultOptions).forEach(
        v => (passed[v] = this.defaultOptions[v].value)
      );
      return passed;
    },
    addNewStep(e) {
      ...
      Api.createNewStep(form)
        ...
        .catch(err => {
          if (err.response.status == 422) {
            ...
            let errs = [];
            Object.keys(this.errorList).forEach(prop => errs = errs.concat(this.errorList[prop]));
